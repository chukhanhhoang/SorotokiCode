    ...
    emxArray_real_T *J, double *Vg, double *Kin)
{
  ...
  for (ii = 0; ii < i; ii++) {
    ...
    /*  update integrands */
    loop_ub = 6 * Z1->size[1];
    i4 = Z1->size[0] * Z1->size[1];
    Z1->size[0] = 6;
    emxEnsureCapacity_real_T(Z1, i4);
    for (i4 = 0; i4 < loop_ub; i4++) {
      Z1->data[i4] += a_tmp * (K1Z1->data[i4] + 3.0 * K2Z1->data[i4]);
    }
    loop_ub = Z2->size[0] * Z2->size[1];
    for (i4 = 0; i4 < loop_ub; i4++) {
      Z2->data[i4] += a_tmp * (K1Z2->data[i4] + 3.0 * K2Z2->data[i4]);
    }
    /*  compute kinematics */
    for (i4 = 0; i4 < 3; i4++) {
      p->data[i4 + 3 * ii] = Z1->data[i4 + 18];
      j = 3 * i4 + 9 * ii;
      Phi->data[j] = Z1->data[6 * i4];
      Phi->data[j + 1] = Z1->data[6 * i4 + 1];
      Phi->data[j + 2] = Z1->data[6 * i4 + 2];
    }
    /* --------------------------------------------------------------------------
     */
    for (i4 = 0; i4 < 3; i4++) {
      loop_ub = i4 + 9 * ii;
      Rt[3 * i4] = Phi->data[loop_ub];
      Rt[3 * i4 + 1] = Phi->data[loop_ub + 3];
      Rt[3 * i4 + 2] = Phi->data[loop_ub + 6];
    }
    /* --------------------------------------------------------------------------
     */
    memset(&b_a[0], 0, 36U * sizeof(double));
    for (i4 = 0; i4 < 3; i4++) {
      s = Rt[3 * i4];
      b_a[6 * i4] = s;
      loop_ub = 6 * (i4 + 3);
    ...
    i4 = K1Z1->size[0] * K1Z1->size[1];
    K1Z1->size[0] = 6;
    K1Z1->size[1] = (i5 - i3) + 1;
    emxEnsureCapacity_real_T(K1Z1, i4);
    for (j = 0; j <= n; j++) {
      loop_ub = j * 6;
      for (b_i = 0; b_i < 6; b_i++) {
        s = 0.0;
        for (k = 0; k < 6; k++) {
          i4 = loop_ub + k;
          s += b_a[k * 6 + b_i] * Z1->data[i4 % 6 + 6 * (i3 + i4 / 6)];
        }
